...
(expect 10 (count (get-entries-generic miles_davis_data b)))

;; test on the first entry's url
(expect "https://en.wikipedia.org/wiki/Miles_Davis"
        (-> miles_davis_data
            (get-entries-generic b )
            first
            (:url)
            )
        )
...
(expect [] (get-entries-generic "this is not a valid bing like json" b    ) )

;; if the data is valid json, but is in a different structure than what I expect from Bing, I will return an empty list
(expect [] (get-entries-generic (slurp "test/cloogling/bad_data_bing_d_or_results.json") b    ))

;; valid json, but does not contain a list of entries
(expect [] (get-entries-generic (slurp "test/cloogling/bad_data_bing_2.json") b     ))

;; valid json, it contains a list of entries in the right place, but the keys are not what expected from Bing
(expect [] (get-entries-generic (slurp "test/cloogling/bad_data_bing_3.json") b    ))
...
;; Counting entries in standard test data
(expect 10 (count (get-entries-generic miles_davis_data_google g   )))

;; test on the first entry's url
(expect "https://en.wikipedia.org/wiki/Miles_Davis"
        (-> miles_davis_data_google
            (get-entries-generic g   )
            first
            (:url)
            )
        )
